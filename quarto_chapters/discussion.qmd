...
Examining multivariate selection responses are just one application relevant to potato breeders. More attention is needed here to make these methods more scalable in applied settings. Traditional linear mixed modelling tends to be inpractical with more than 10 variates making other methods such as Bayesian learning more attractive [@Blows2009; @Runcie2013].
...
There are three areas of study which deserve more careful attention in terms of guaranteeing the success of hybrid potato with further applied quantitative genetics.
...
Often, the question asked of genomic selection applications in crop breeding are "_What kind of gains can we expect with the implementation of genomic prediction"_? While this is not unsound in of itself, it ought to be paired with "_What are the risks of a genomic prediction implementation_"? This addendum is especially pertinent for potato breeders for the following reasons. It is not unusual that  . Formalizing the estimation of risk explicitly really is the province of quantitative genetics but is too often under-emphasised. be asked in parallel Risk-based approach for estimating potato sensitivity.
