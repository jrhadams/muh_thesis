...
If breeders begin to focus on traits related seed production and on the utilization of CMS systems in potato, multi-pool hybrid breeding might very well be possible.
...
Genomic prediction in potato promises to accelerate breeding gains in the coming decades. **Chapter's 3, 4, and 5** have several implications for genomic prediction applications in potato. Beginning with model choice (including factors such as parameterization of genetic model and modelling framework), is not the _decisive_ factor in the success of a genomic prediction strategy. Having reviewed traditional GBLUP methods, 
