...
A core concept in hybrid breeding is the ability to assess the potential of hybrid crosses on the basis of parental value. In the case for phenotypic selection, general combining abilities tend to be the primary mode which communicates a line's value [@]. In **chapter 2** we generated GCAs for 400 parental lines in multiple tuber variates and found that the GCAs sufficiently captured a large portion of the phenotypic variation in the F1 screening trials. The use of GCA for selection is not unique to hybrid crops with many examples of their use in tetraploid potato [@Neele1991; @Bradshaw2003]. The one caveat here is that the success of GCAs in selection are dependent on a large GCA variance in your crosses which is dependent on the trait architecture, the crossing design, and population composition. Multiple tetraploid studies found SCA being the primary genetic effect controlling tuber size, tuber yield, and marketable yield [@Plaisted1962; @Tai1976 ;@Killick1977]. This outlines the potential increased efficiency of phenotypic selection.
