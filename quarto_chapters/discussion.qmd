...
This would suggest a mean increase with This of course could be transformed into an index using a variety of methods. More attention is needed here to make these methods more scalable in applied settings. Traditional linear mixed modelling tends to be inpractical with more than 10 variates making other methods such as Bayesian learning more attractive [@Blows2009; @Runcie2013].
...
Often, while considering a novel technology or more efficient breeding schema, the first question is "_What kind of genetic gains can be expected with this implementation"_? This in of itself is not unsound, however  ought to be paired with "_What are the risks of a genomic prediction implementation_"? This addendum is especially pertinent for potato breeders for the following reasons. It is not unusual that  . Formalizing the estimation of risk explicitly really is the province of quantitative genetics but is too often under-emphasised. @fig-risk be asked in parallel Risk-based approach for estimating potato sensitivity.
