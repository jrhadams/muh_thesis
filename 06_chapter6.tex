...
tuber-sown hills\textsuperscript{{[}8,9{]}}. 
The latter tend to be underpowered, with insufficient precision to support early selection. Thus, true potato seed could be a powerful alternative resource in breeding germplasm evaluation. \textsuperscript{{[}10,11{]}}. This is not without negative
...
This is worth re-emphasis as there are many complex traits in potato which must undergo selection together in a breeding program \textsuperscript{{[}27{]}}. The capacity to decompose the relationships between these correlated traits offers insight to breeders. Additionally, hybrid breeding schemas could benefit from such information to aid in breeding schema design and pool development strategy. Multivariate modelling can also augment genomic prediction in hybrid breeding schemas making it a valuable tool both for inference and predictive applications\textsuperscript{{[}28{]}}.
...
GCA+SCA, respectively) for the same tuber traits from \textbf{chapter
...
addition of the SCA component in the model. Additionally, the magnitude of genetic residual variance relative to the SCA variance suggested other possible higher-order genetic interactions,
most notably in total tuber number and dry matter content. Despite this, genomic prediction was sufficiently powerful solely on the basis of GCA in the prediction of a hybrid's genetic value.
...
parents of hybrids along with multiallelic identity-by-state (IBS)
...
along with more complicated kernel prediction. The performance of SNP-based prediction models were superior over their multiallelic
counterparts for all traits. Marked differences in prediction accuracy were especially
apparent between the SNP and IBD models. Similarly, little differences
...
variance tends to be more present in a complex trait architecture where
heterotic pools are not genetically distinct\textsuperscript{{[}31{]}}.
Contrary to this, my results follow the more general pattern where the dominance variance appears to be nothing
more than a sort of \emph{"residual"} with most of the genetic variation
...
selection among the other traits. This is invaluable both in
...
hybrid crops\textsuperscript{{[}53{]}}. \textbf{Chapters' 3, 4, and 5}
...
hybrid crosses. \textbf{Chapters' 3, 4, and 5} all utilized four field
...
genetic variation can be interrogated by some ideal probeset. While
this cannot be fully achieved\textsuperscript{{[}66{]}}, molecular marker
composition is a worthwhile question in efficient selection
applications. In \textbf{Chapter 4}, I considered a procedure for
...
could be effectively selected for with only a handful of molecular
...
inbreds to initialize a breeding program is to be seen, but it is a tool
